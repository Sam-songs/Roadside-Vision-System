...
class HeatmapWorker(QtCore.QThread):
    ...
    def run(self):
        try:
            ...
            if not cap.isOpened():
                self.started_ok.emit(False, f"无法打开视频: {self.video_path}")
                return
            ...
            canvas_rgba = None
            frame_id = 0
            self.started_ok.emit(True, "热力图已启动")

            while not self._stop_flag:
                ok, im0 = cap.read()
                if not ok:
                    self.progress.emit("视频读取完毕。")
                    break
                ...
                if frame_id % SAVE_EVERY == 0:
                    ...
                    if ok_enc:
                        ...
                        self.progress.emit("PNG 编码失败（跳过本帧保存）。")

                frame_id += 1
                if frame_id % 10 == 0:
                    self.progress.emit(f"已处理帧: {frame_id}")

            cap.release()
        except Exception as e:
            self.started_ok.emit(False, f"运行异常: {e}")
        finally:
            self.finished.emit()
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _build_ui(self):
        ...
        btn_v = self._btn("浏览…", self._browse_video)
        form.addRow("视频路径", self._hbox(self.edit_video, btn_v))

        self.edit_model = self._line(DEFAULT_MODEL_PATH)
        btn_m = self._btn("浏览…", self._browse_model)
        form.addRow("模型路径", self._hbox(self.edit_model, btn_m))

        self.edit_outdir = self._line(DEFAULT_OUT_DIR)
        btn_od = self._btn("选择目录…", self._browse_outdir)
        form.addRow("输出目录", self._hbox(self.edit_outdir, btn_od))

        self.edit_outfile = self._line(DEFAULT_OUT_FILE)
        form.addRow("输出文件", self.edit_outfile)
        ...
        form.addRow("交通类别", self.alias_edit)

        # deblur (optional)
        self.chk_deblur = QtWidgets.QCheckBox("是否开启增强")
        self.chk_deblur.setChecked(DEFAULT_DEBLUR)
        self.edit_deblur = self._line(DEFAULT_DEBLUR_WEIGHTS)
        btn_dw = self._btn("权重…", self._browse_deblur)
        form.addRow(self.chk_deblur)
        form.addRow("增强权重", self._hbox(self.edit_deblur, btn_dw))

        # buttons
        btn_row1 = self._hbox(
            self._btn("开始车道判别", self.start_heatmap),
            self._btn("停止车道判别", self.stop_heatmap),
            self._btn("打开输出目录", self.open_out_dir)
        )
        left.addWidget(btn_row1)

        btn_row2 = self._hbox(
            self._btn("处理输出车道", self.run_post_process),
            self._btn("导出监测视频", self.start_lane),
            self._btn("停止导出", self.stop_lane)
        )
        ...
        self.preview_tip = QtWidgets.QLabel("预览")
    ...
    def _browse_model(self):
        path, _ = QtWidgets.QFileDialog.getOpenFileName(self, "选择模型", "", "Weights (*.pt *.onnx *.engine);;All Files (*)")
    ...
    def _browse_deblur(self):
        path, _ = QtWidgets.QFileDialog.getOpenFileName(self, "选择去模糊权重", "", "Weights (*.h5 *.pth *.pt)")
    ...
    def stop_heatmap(self):
        if self.hm_worker and self.hm_worker.isRunning():
            self.hm_worker.stop(); self._log("停止车道判别请求已发送…")
    ...
    def refresh_preview_png(self):
        ...
        try:
            ...
            self.statusBar().showMessage(f"已刷新：{os.path.basename(path)}")
    ...
    def run_post_process(self):
        ...
        if not os.path.isfile(img_path):
            self._log(f"未找到输出图片：{img_path}"); return
        try:
            ...
            self._log(f"[后处理完成] JSON: {json_path}")
            self._log(f"[后处理完成] VIS:  {vis_path}")
    ...
    def start_lane(self):
        ...
        self.preview_tip.setText("预览：正在显示实时输出的视频帧")
    ...
    def stop_lane(self):
        if self.ln_worker and self.ln_worker.isRunning():
            self.ln_worker.stop(); self._log("停止车道导出请求已发送…")
    ...
    def _collect_lane_config(self) -> Optional[dict]:
        ...
        if not os.path.isfile(polygons_json):
            self._log("未找到 polygons.json，请先点击‘处理输出图片’或手动提供。"); return None
        ...
        self._log(f"[Lane Export] 使用 JSON: {polygons_json}")
        self._log(f"[Lane Export] 输出: {video_out}")
        return cfg
    ...
    def _on_lane_finished(self):
        self._log("[OK] 车道视频导出完成。")
        self.preview_tip.setText("预览：导出完成。")
        self._realtime_preview = False   # 恢复 PNG 轮询预览

    @QtCore.pyqtSlot(str)
    def _on_lane_failed(self, err: str):
        self._log(f"[ERROR] {err}")
        self.preview_tip.setText("预览：导出失败。")
